...
const broadcastIncidents = () => {
  let incidentsData = tempDB.get("incidents").value();
  io.emit("incidents_feed", { data: incidentsData });
  console.log("Core : Broadcasted Incidents data!");
};
...
app.get("/events", (req, res) => {
  axios.get(`https://api.covid19api.com/country/philippines`).then((result) => {
    ...
    let countryStatus = {
      ...
      image_url:
        "https://blc.edu/wp-content/uploads/2020/03/COVID19-graphic-with-text-FEATURED-IMAGE.jpg",
      subtitle: `As of ${moment(phData.Date).format(
        "MMMM D, YYYY"
      )} latest updates.`,
    };
...
app.post("/signup", (req, res) => {
  if (req.body.email && req.body.password && req.body.name && req.body.city) {
    let userInstance = tempDB
      .get("users")
...
app.get("/public/responder", (req, res) => {
  if (req.query.responderId) {
    let responderInstance = tempDB
      .get("users")
      .find({ id: req.query.responderId })
      .value();
    let cancelledReports = tempDB
      .get("incidents")
      .filter({ status: "cancelled" })
      .value();
    let responderHistory = tempDB
      .get("incidents")
      .filter({ responder: req.query.responderId })
      .value();
...
const cancelReport = (uid) => {
  if (reportees) {
    if (reportees[uid]) {
      ...
      let reportInstance = tempDB
        .get("incidents")
        .find({ uid: reportees[uid].uid })
        .value();
...
const handleMessage = (sender_psid, received_message, attachments) => {
  ...
  if (report) {
    ...
    if (report.type !== "" && report.details !== "" && report.location === "") {
