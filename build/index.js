...
const ulitilies_1 = require("./ulitilies");
...
app.get("/incidents", (req, res) => {
    res.send(ulitilies_1.tempDB.get("incidents").value());
});
app.get("/events", (req, res) => {
    res.send(ulitilies_1.tempDB.get("events").value());
});
app.post("/login", (req, res) => {
    ...
    if (!email || !password) {
        ...
        let userInstance = ulitilies_1.tempDB.get("users").find({ email: email }).value();
        if (userInstance) {
...
app.post("/signup", (req, res) => {
    if (req.body.email && req.body.password
        ...
        let userInstance = ulitilies_1.tempDB.get("users").find({ email: req.body.email }).value();
        if (userInstance) {
            res.send({
                status: "failed",
            ...
            bcrypt.hash(req.body.password.trim(), 10, (err, hash) => {
                ...
                ulitilies_1.tempDB.get("users").value().push(newUser);
                ulitilies_1.tempDB.write();
                res.send({
                    status: "success",
...
app.get("/public/responder", (req, res) => {
    if (req.query.responderId) {
        let responderInstance = ulitilies_1.tempDB.get("users").find({ id: req.query.responderId }).value();
        let cancelledReports = ulitilies_1.tempDB.get("incidents").filter({ status: "cancelled" }).value();
        let responderHistory = ulitilies_1.tempDB.get("incidents").filter({ responder: req.query.responderId }).value();
    ...
    return {
        "attachment": {
            ...
            "payload": {
                ...
                "text": messages_1.default.welcomeGreeting,
                "buttons": [
                    {
                        "type": "postback",
...
const cancelReport = (uid) => {
    if (reportees) {
        if (reportees[uid]) {
            sendMessage(uid, messages_1.default.cancelReport);
            let reportInstance = ulitilies_1.tempDB.get('incidents').find({ uid: reportees[uid].uid });
            console.log(`
       ================
       ${reportInstance}
       ===============
...
const sendRecentEvents = (sender) => {
    let sampleEvents = ulitilies_1.tempDB.get("events").value();
    let events = {
        "attachment": {
            "type": "template",
            "payload": {
...
const askForLocation = (sender) => {
    let options = {
        "attachment": {
            ...
            "payload": {
                "template_type": "button",
                "text": messages_1.default.askLocation,
                "buttons": [
                    {
                        "type": "postback",
                        "title": "Cancel",
...
const getLocation = (sender) => {
    let options = {
        "attachment": {
            ...
            "payload": {
                "template_type": "button",
                "text": messages_1.default.getLocation,
                "buttons": [
                    {
                        "type": "postback",
                        "title": "Cancel",
...
const askforCompletetion = (sender) => {
    let options = {
        "attachment": {
            ...
            "payload": {
                "template_type": "button",
                "text": messages_1.default.incompleteReport,
                "buttons": [
                    {
                        "type": "postback",
                        "title": "Cancel Report",
...
const getAreas = (sender, location) => {
    axios.get(`https://maps.googleapis.com/maps/api/place/nearbysearch/json`, {
        ...
        if (res.data.results.length > 0) {
            sendMessage(sender, {
                "text": messages_1.default.nearbyEvacuation
            });
            let AreaList = {
                "attachment": {
                    "type": "template",
            ...
            sendMessage(sender, {
                "text": messages_1.default.noAreas
            });
        }
...
const getEvacuationAreas = (sender) => {
    let message = {
        "text": messages_1.default.areaAskLocation,
    };
    sendMessage(sender, message);
    message = {
        "text": messages_1.default.expectLocation
    };
    sendMessage(sender, message);
...
const incidentReport = (sender) => {
    let list1 = {
        "attachment": {
            ...
            "payload": {
                "template_type": "button",
                "text": messages_1.default.askIncident,
                "buttons": [
                    {
                        "type": "postback",
    ...
    let list2 = {
        "attachment": {
            ...
            "payload": {
                "template_type": "button",
                "text": messages_1.default.askProblem,
                "buttons": [
                    {
                        "type": "postback",
                        "title": "Sunog (Fire)",
...
const getDetails = (sender) => {
    sendMessage(sender, {
        "text": messages_1.default.getDetails
    });
};
const getSpecification = (sender) => {
    sendMessage(sender, {
        "text": messages_1.default.getSpecification
    });
};
const handleMessage = (sender_psid, received_message, attachments) => {
    ...
    if (report) {
        ...
        if (report.type !== ""
            ...
            if (attachments && attachments[0].type === "location") {
                report.location = ulitilies_1.getCoordinates(attachments[0].payload.url);
                let messageText = messages_1.default.gotLocation;
                ...
                ulitilies_1.tempDB.get('incidents').value().push(newReport);
                ulitilies_1.tempDB.write();
            }
        }
    ...
    if (evacuees.includes(sender_psid)) {
        if (attachments && attachments[0].type === "location") {
            let messageText = messages_1.default.waitforFacilities;
            sendMessage(sender_psid, { "text": messageText });
            // broken due to messenger webhook version 6.0
            // getAreas(sender_psid, attachments[0].payload.coordinates)
            getAreas(sender_psid, ulitilies_1.getCoordinates(attachments[0].payload.url));
        }
        else {
...
const handlePostback = (sender_psid, received_postback) => {
    ...
    if (payload === '') {
        response = textResponse(messages_1.default.goAhead);
        sendMessage(sender_psid, response);
    }
    else {
        switch (payload) {
            ...
            case 'cancel_evacuation':
                if (evacuees) {
                    ...
                    sendMessage(sender_psid, { "text": messages_1.default.cancelLocation });
                    sendMessage(sender_psid, defaultActions());
                }
                break;
            case 'cancel_report':
                if (reportees[sender_psid]) {
                    sendMessage(sender_psid, { "text": messages_1.default.cancelReport });
                    sendMessage(sender_psid, defaultActions());
...
io.sockets.on('connection', function (socket) {
    socket.on("verifyReport", (data) => {
        ...
        sendMessage(data.id, {
            "text": messages_1.default.reportVerified
        });
        sendMessage(data.id, {
            "text": messages_1.default.stayOnline
        });
        let reportInstance = ulitilies_1.tempDB.get('incidents').find({ uid: data.uid }).value();
